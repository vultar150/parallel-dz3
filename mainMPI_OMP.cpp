...
public:
    Process(uint32_t Nx, uint32_t Ny, uint32_t Nz, int rank, int num_procs[], int coords[], 
            double tau, uint32_t steps, MPI_Comm GRID_COMM):
                curr_block(Nx, Ny, Nz), 
                p_block(Nx, Ny, Nz), 
                pp_block(Nx, Ny, Nz),
...
private:
    ...
    double GetEps(uint32_t ln, double t) {
        double max_eps = 0.0;
        double tmp = 0.0;
        uint32_t s_i = coords[0] == 0 ? 0 : 1;
        uint32_t s_j = coords[1] == 0 ? 0 : 1;
        uint32_t s_k = coords[2] == 0 ? 0 : 1;
     
        #pragma omp parallel for reduction(max: max_eps)
        for (uint32_t i = s_i; i < Nx - 1; i++) {
            for (uint32_t j = s_j; j < Ny - 1; j++) {
                for (uint32_t k = s_k; k < Nz - 1; k++) {
...
int main(int argc, char **argv) {
    ...
    Process P(Nx, Ny, Nz, rank, num_procs, coords, T / (double) K, STEPS, GRID_COMM);
    P.Solve();
